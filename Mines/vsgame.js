var user = "";
var pass = "";
var URL = "http://twserver.alunos.dcc.fc.up.pt:8037/";
var game_num = null;
var game_key = null;
var opponent = false;
var quemJoga = null;
...
function createvsgame() {
	nbombs = maxbombs = diff[difficulty].nbombs;
	...
	req.onreadystatechange = function() {
		if (req.readyState == 4 && req.status == 200) {
			...
			if (response["error"] == undefined) {
				...
				var link = 'http://twserver.alunos.dcc.fc.up.pt:8037/update?name='
						+ user + '&game=' + game_num + '&key=' + game_key;
				...
				source.addEventListener('message', function(e) {
					...
					if (ansL["opponent"] != undefined) {
						...
						document.getElementById('loading').classList
								.add('hidden');
						cl.hide();
						document.getElementById('players').classList
								.remove('hidden');
						document.getElementById('players').appendChild(
								document.createTextNode(user + " VS "
										+ ansL["opponent"]));
					}
					...
					if (ansL["turn"] != undefined) {
						quemJoga = ansL["turn"];
						if (quemJoga == user) {
							document.getElementById('turn').classList
									.add('green');
							document.getElementById('turn').classList
									.remove('red');
							document.getElementById('turn').classList
...
function expandeX(array, table) {
	for ( var i = 0; i < array.length; i++) {
		if (!(array[i][2] === 0 || array[i][2] == -1)) {
			table.rows[array[i][0] - 1].cells[array[i][1] - 1]
					.appendChild(document.createTextNode(array[i][2]));
		}
		if (array[i][2] == -1) {
			...
			if (array[i][2] === 1)
				table.rows[array[i][0] - 1].cells[array[i][1] - 1].classList
						.add('blue');
			else if (array[i][2] === 2)
				table.rows[array[i][0] - 1].cells[array[i][1] - 1].classList
						.add('green');
		...
		if (req.readyState == 4 && req.status == 200) {
			...
			if (response["ranking"] != undefined) {
				...
				for ( var i = 0; i < outroA.length; i++) {
					...
					var node = document.createTextNode(outroA[i].name + " "
							+ outroA[i].score);
...
function init() {
	...
	ctx.strokeStyle = color;
	ctx.lineWidth = 30;
	ctx.arc(W / 2, H / 2, 100, 0 - 90 * Math.PI / 180, radians - 90 * Math.PI
			/ 180, false);
	ctx.stroke();
	ctx.fillStyle = color;
	ctx.font = "50px bebas";
...
function init2() {
	...
	ctx2.strokeStyle = color2;
	ctx2.lineWidth = 30;
	ctx2.arc(W2 / 2, H2 / 2, 100, 0 - 90 * Math.PI / 180, radians2 - 90
			* Math.PI / 180, false);
	ctx2.stroke();
...
function draw2() {
	if (typeof animation_loop2 != undefined)
